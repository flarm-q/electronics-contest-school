...
void EXTI9_5_IRQHandler(void)
{
	...
	if(EXTI_GetITStatus(EXTI_Line5)!=0)//一级判定
	{
		if(PBin(5)==0)//二级判定
		{
			...
			//1.5、二开部分
			/*********************************************************************************************/
			/* 前后控制：收到八路巡线有效帧后，速度由巡线模块接管；否则保持原蓝牙控制逻辑。 */
			if(Tracking_IsActive())
			{
				Tracking_SetSpeed();
			}
			else
			...
			/* 左右控制：巡线模式下禁止蓝牙左右指令继续叠加，避免两套转向目标互相冲突。 */
			if(Tracking_IsActive())Turn_Speed=0;
			else if((Left==0)&&(Right==0))Turn_Speed=0;
			...
			/* 转向约束：巡线模式使用红外偏差 + Z 轴陀螺仪转向 PD，不再使用原 Turn_Kd 约束。 */
			if(Tracking_IsActive())Turn_Kd=0;
			else if((Left==0)&&(Right==0))Turn_Kd=0.6;//若无左右转向指令，则开启转向约束
			...
			if(Tracking_IsActive())Turn_out=Tracking_TurnPD(gyroz);//巡线模式：根据八路红外偏差生成差速转向
			else Turn_out=Turn(gyroz,Turn_Speed);																//转向环
